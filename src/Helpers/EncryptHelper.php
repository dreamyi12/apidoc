...
class EncryptHelper {
    ...
    public static function authcode(string $data, string $key, bool $encode = true, int $expiry = 0): array {
        ...
        // 密钥c用于变化生成的密文
        $keyc      = $encode ? substr(md5(microtime()), -8) : substr($data, 0, 8);
        $keyd      = md5($keya . $keyc);
        $cryptkey  = $keya . $keyd;
        ...
        if ($encode) {
            ...
            $data = self::base64UrlDecode(substr($data, 8));
        }
    ...
    public static function easyEncrypt(string $data, string $key): string {
        ...
        return substr($key, 0, 8) . self::base64UrlEncode($str);
    }
    ...
    public static function easyDecrypt(string $data, string $key): string {
        ...
        $key = md5($key);
        if (substr($key, 0, 8) != substr($data, 0, 8)) {
            return '';
        }
